...
export default function Experience() {
  ...
  // A project card elsewhere on the site can ask this section to jump to
  // and highlight the job it came from — see hooks/useSectionNav.js.
  useEffect(() => {
    const onFocusExperience = (e) => {
      const idx = experience.findIndex((job) => job.id === e.detail)
      if (idx !== -1) setActiveIdx(idx)
    }
    window.addEventListener('portfolio:focus-experience', onFocusExperience)
    return () => window.removeEventListener('portfolio:focus-experience', onFocusExperience)
  }, [])
      ...
      <div ref={ref} className={visible ? 'animate-fade-in-up' : 'opacity-0'}>
        ...
        <div className="flex flex-col gap-6 sm:flex-row">
          ...
          <div key={activeIdx} className="flex-1 animate-fade-in">
            ...
            {job.roleDetail && (
              <p className="mt-0.5 text-sm italic text-stone-500">{job.roleDetail}</p>
            )}
            <p className="mt-1 font-mono text-xs text-stone-500">
              {job.period} · {job.location}
            </p>
            <p className="mt-4 text-sm text-stone-400">{job.blurb}</p>

            <ul className="mt-4 space-y-3">
              {job.achievements.map((a, i) => (
                <li key={i} className="flex gap-3 text-sm leading-relaxed text-stone-300">
